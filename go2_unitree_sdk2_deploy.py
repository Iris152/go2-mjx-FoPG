...
def parse_args() -> argparse.Namespace:
    ...
    # 阶段持续时间：先起身、站稳，再进入策略。
    # 真机默认使用实测过的短 policy ramp；进入策略后不启用低通滤波。
    ...
    # 写入 LowCmd 的 PD 增益。仿真和真机默认都使用实测过的 policy_kd=0.5。
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "When domain_id=0, use the real-robot-tested defaults for policy ramp "
